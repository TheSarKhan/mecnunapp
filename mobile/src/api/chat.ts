...
export async function sendMessage(
  ...
  const { data } = await api.post<SendMessageResponse>('/chat/messages', { mode, content, conversationId });
  return data;
}
...
export async function getMessages(conversationId: string): Promise<MessageDto[]> {
  const { data } = await api.get<MessageDto[]>(`/chat/conversations/${conversationId}/messages`);
  return data;
}
